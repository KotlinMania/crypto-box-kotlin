...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `CryptoBox.swiftmodule/`
...
//      `import CryptoBox` above would fail at compile time.
//
//   2. The static archive `libCryptoBox.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "CryptoBox" }` and
//      `flattenPackage = "io.github.kotlinmania.cryptobox"` configuration in
...
//      via the `CryptoBoxLibrary` product.
//
...
final class CryptoBoxExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "CryptoBox swift module imported cleanly")
    }
}
