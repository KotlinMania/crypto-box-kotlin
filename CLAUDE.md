# Claude Code Project Instructions — crypto-box-kotlin

## Project Overview

This is **crypto-box-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`crypto_box`](https://crates.io/crates/crypto_box).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/crypto_box/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/crypto_box>
